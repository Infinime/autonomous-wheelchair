...
function App() {
    ...
    const isConnectedToEsp32 = async () => {
        ...
        fetch("http://" + espIp + "/", {
            ...
            .catch((error) => {
                alert("An error has occured when trying to connect to the ESP, here's some more info.", error);
                setIsConnected(false);
            ...
            <div className="container h-screen w-screen m-auto overflow-hidden p-0">
                <div className=" flex flex-col h-full   pt-3 justify-end gap-10  shrink-0 items-center">
                    <div className="w-11/12 lg:w-8/12 h-fit flex flex-row gap-1  pt-3 align-middle items-center p-2 bg-zinc-300 rounded-lg lg:rounded-[18px] border border-black">
                        ...
                        <div className="  text-black lg:text-xl font-bold font-['Inter'] leading-none">72%</div>
                        <div className={" w-5 h-5 lg:w-6 lg:h-6 lg:ml-5 rounded-full" + (isConnected ? " bg-emerald-800 " : " bg-red-600")} />
                        <div className="  text-black lg:text-2xl font-bold font-['Inter'] leading-none">{isConnected ? " Connected" : "Disconnected"}</div>
                        <span className="material-symbols-outlined ml-1 lg:ml-8">conversion_path</span>
                        <div className=" text-black lg:text-2xl font-bold font-['Inter'] leading-none">Status:</div>
                        <h4 className=" text-sm lg:text-lg font-medium">{status}</h4>
                    </div>
                    {!isConnected ? (
                        <div className="flex flex-row gap-2 justify-center items-center">
                            <input
                                type="text"
                                className="w-4/12 h-fit border-black border rounded-md"
                                value={espIp}
                                onChange={(e) => {
                                    console.log(e.target.value);
                                    setEspIp(e.target.value);
                                }}
                                placeholder="IP from ESP"
                                id="inputBtn"
                            />

                            <button className="border border-black  bg-emerald-600  text-white mt-4 mb-4  rounded-md" onClick={isConnectedToEsp32}>
                                Connect to ESP32
                            </button>
                        </div>
                    ) : null}
                    <div className=" mt-10 flex flex-col items-center">
                        <div className="flex flex-row mb-2 transition">
                            <button className="h-fit" onClick={!isRecording ? startRecording : stopRecording}>
                                <img className=" transition ease-linear" src={!isRecording ? recordButton : stopButton} width={"90px"} />
                            </button>
                            {isRecording ? (
                                <button className="h-fit ml-3" onClick={togglePauseResume}>
                                    <img className=" transition ease-linear" src={!isPaused ? pauseButton : resumeButton} width={"90px"} />
                                </button>
                            ) : null}
                        </div>
                        <p className=" text-center text-black text-3xl font-semibold font-['Roboto'] leading-none">{!isRecording && !isPaused ? "Record" : parseTime(recordingTime)}</p>
                    </div>
                    ...
                    <div className="lg:w-3/12 bg-zinc-300 h-fit rounded-t-3xl lg:rounded-t-[43px] items-center border border-black flex flex-col mt-auto gap-1 pt-0">
                        <div className=" text-black text-[32px] font-medium font-['Roboto'] leading-none mt-4 mx-2">Commands List</div>
                        <div className="flex flex-row flex-wrap justify-center items-center align-middle p-0 w-full pl-3 pr-3  h-4/6">
                            ...
                            <ol className="text-xl text-left flex flex-col w-full">
                                <li className=" border-b border-black bg-[#b1afaf]">Move Forward</li>
                                <li className=" border-b border-black bg-[#b1afaf]">Move Backward</li>
                                <li className=" border-b border-black bg-[#b1afaf]">Turn Left</li>
                                <li className=" border-b border-black bg-[#b1afaf]">Turn Right</li>
                                <li className=" border-b border-black bg-[#b1afaf]">Stop</li>
                                <li className=" border-b border-black bg-[#b1afaf]">Slow Down</li>
                                <li className=" border-b border-black bg-[#b1afaf]">Speed Up</li>
                            </ol>
